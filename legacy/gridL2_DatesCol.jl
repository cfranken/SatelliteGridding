...
function favg_all!(arr,lat,lon,inp,s,s2,n, latMin, latMax, lonMin, lonMax, nLat, nLon, points)
    ...
    @inbounds for i in 1:s
		...
		if (dimLat[i]==1) & (dimLon[i]==1)
			for z in 1:s2
	                arr[z,iLon[i,1],iLat[i,1]] +=  fac*inp[z,i]
	        end
...
function main()
    ...
    # Define data array
    mat_data= zeros(Float32,(1+length(dGrid),length(lon),length(lat)))
	...
	for d in startDate:dDay:stopDate
	    ...
	    for a in files[fileSize.>0]
	        ...
	        if length(idx) > 0
				#print(size(lat_in_))
	            mat_in =  zeros(Float32,(length(dGrid)+1),length(lat_in_[:,1]))
				dim = size(mat_in)
				...
				@time  favg_all!(mat_data, iLat_,iLon_,mat_in[:,idx],length(idx),dim[1],nGrid, latMin, latMax, lonMin,lonMax, length(lat), length(lon), points )
	        ...
	        close(fin)
			#catch
		#		println("Error in file caught ", backtrace())
			#end
	    end
		...
		co = 1
		thr = 5
		#viewN =  view(mat_data, end, :, :)
		#valInd = findall(x->x>0,viewN)
		for (key, value) in dGrid
