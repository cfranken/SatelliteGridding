...
function aver!(mat_data, mat_in, iLon_,iLat_, idx)
	...
	#println(eltype(mat_in), " Haha")
	limAlb = FT(0.03/0.0001)
	@inbounds for i = 1:length(idx)
		...
		for j=1:d[end]-1
            #println(j)
            #_t = mat_in[idx[i][2],idx[i][1],j];
			A[j] = mat_in[idx[i][2],idx[i][1],j];
		end
		#println(sum(A))
		if sum(A)<FT(229369)
		# if !any(A.==32767)
			...
			if(A[2]>limAlb)
				...
				for j=1:d[end]-1
					@inbounds mat_data[iLon_[i],iLat_[i],j] +=  A[j]*FT(0.0001)
				end
				# Compute vegetation indices here:
				# EVI
				@inbounds mat_data[iLon_[i],iLat_[i],d[end]] +=G*(A[2]-A[1])/(A[2]+C1*A[1]-C2*A[3]+L)
				# NDVI
				@inbounds NDVI[1] = (A[2]-A[1])/(A[2]+A[1])
				@inbounds mat_data[iLon_[i],iLat_[i],d[end]+1] += NDVI[1]

				# NIRv
				@inbounds mat_data[iLon_[i],iLat_[i],d[end]+2] += NDVI[1]*A[2]*0.0001
				# NWDI
				@inbounds mat_data[iLon_[i],iLat_[i],d[end]+3] +=(A[2]-A[5])/(A[2]+A[5])
				# N
				@inbounds mat_data[iLon_[i],iLat_[i],end] +=   mat_in[idx[i][2],idx[i][1],end]
			end
		end
	end
end
...
function main()
	...
	for (key, value) in dGrid
		print(key," ")
		NCDict[key] = defVar(dsOut,key,Float32,("lon","lat", "time"),deflatelevel=4, fillvalue=-999)
	end
	ds_ndvi = defVar(dsOut,"NDVI",Float32,("lon","lat", "time"),deflatelevel=4, fillvalue=-999)
	ds_evi  = defVar(dsOut,"EVI",Float32,("lon","lat", "time"),deflatelevel=4, fillvalue=-999)
	ds_nirv = defVar(dsOut,"NIRv",Float32,("lon","lat", "time"),deflatelevel=4, fillvalue=-999)
	ds_ndwi = defVar(dsOut,"NDWI",Float32,("lon","lat", "time"),deflatelevel=4, fillvalue=-999)

	println(" ")
	#dSIF = defVar(dsOut,"sif",Float32,("lon","lat"),deflatelevel=4, fillvalue=-999)
	dN = defVar(dsOut,"n",Float32,("lon","lat", "time"),deflatelevel=4, fillvalue=-999)
    # Define data array
	...
	#mat_in2 =  zeros(Float32,2400,2400,length(dGrid)+1)

	ds = Dataset("/export/home1/pgsql/modis/modis_geotable.nc","r")
	...
	for d in startDate:dDay:stopDate
	    ...
	    for a in files
			...
			#try
				fin = Dataset(a)
				# Just to catch bad data (hopefully)
				_test = fin["Nadir_Reflectance_Band1"].var
