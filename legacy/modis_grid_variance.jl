...
function aver!(oldM, newM, oldS, newS, mat_in, iLon_,iLat_, idx)
	NDVI = zeros(Float32,1)
	d = size(mat_in)
	for i = 1:length(idx)
		A = view(mat_in,idx[i][2],idx[i][1],1:d[3]-1)
		if !any(A.==32767)
			# NIR reflectance > 0.01, gets rid of water
			n = oldM[iLon_[i],iLat_[i],end]+1
			if n==1
				for j=1:d[3]-1
					oldM[iLon_[i],iLat_[i],j] =  mat_in[idx[i][2],idx[i][1],j]*0.0001
					newM[iLon_[i],iLat_[i],j] =  mat_in[idx[i][2],idx[i][1],j]*0.0001
					oldS[iLon_[i],iLat_[i],j] = 0.0
					newS[iLon_[i],iLat_[i],j] = 0.0
				end
				# Compute vegetation indices here:
				# EVI
				oldM[iLon_[i],iLat_[i],d[3]] =G*(A[2]-A[1])/(A[2]+C1*A[1]-C2*A[4]+L)
				newM[iLon_[i],iLat_[i],d[3]] =G*(A[2]-A[1])/(A[2]+C1*A[1]-C2*A[4]+L)
				oldS[iLon_[i],iLat_[i],d[3]] = 0.0
				newS[iLon_[i],iLat_[i],d[3]] = 0.0

				# NDVI
				NDVI[1] = (A[2]-A[1])/(A[2]+A[1])
				#println(NDVI, " ", A[2]*0.0001, " ", A[1]*0.0001, " ", NDVI[1]*A[2]*0.0001," ", (A[2]*0.0001-A[1]*0.0001)/(A[2]*0.0001+A[1]*0.0001))

				oldM[iLon_[i],iLat_[i],d[3]+1] = NDVI[1]
				newM[iLon_[i],iLat_[i],d[3]+1] = NDVI[1]
				oldS[iLon_[i],iLat_[i],d[3]+1] = 0.0
				newS[iLon_[i],iLat_[i],d[3]+1] = 0.0
				#println(mat_data[iLon_[i],iLat_[i],d[3]+1] ," ", mat_data[iLon_[i],iLat_[i],2] )
				# NIRv
				oldM[iLon_[i],iLat_[i],d[3]+2] = NDVI[1]*A[2]*0.0001
				newM[iLon_[i],iLat_[i],d[3]+2] = NDVI[1]*A[2]*0.0001
				oldS[iLon_[i],iLat_[i],d[3]+2] = 0.0
				newS[iLon_[i],iLat_[i],d[3]+2] = 0.0
				# NWDI
				oldM[iLon_[i],iLat_[i],d[3]+3] =(A[2]-A[5])/(A[2]+A[5])
				newM[iLon_[i],iLat_[i],d[3]+3] =(A[2]-A[5])/(A[2]+A[5])
				oldS[iLon_[i],iLat_[i],d[3]+3] = 0.0
				newS[iLon_[i],iLat_[i],d[3]+3] = 0.0
				# N

			else
				for j=1:d[3]-1
					x = mat_in[idx[i][2],idx[i][1],j]*0.0001
					newM[iLon_[i],iLat_[i],j] = oldM[iLon_[i],iLat_[i],j] + (x-oldM[iLon_[i],iLat_[i],j])/n
					newS[iLon_[i],iLat_[i],j] = oldS[iLon_[i],iLat_[i],j] + (x-oldM[iLon_[i],iLat_[i],j])*(x-newM[iLon_[i],iLat_[i],j])
					#println(newM[iLon_[i],iLat_[i],j], " ", sqrt(newS[iLon_[i],iLat_[i],j]), " ", oldM[iLon_[i],iLat_[i],end])
					oldM[iLon_[i],iLat_[i],j] = newM[iLon_[i],iLat_[i],j]
					oldS[iLon_[i],iLat_[i],j] = newS[iLon_[i],iLat_[i],j]
				end
				# Compute vegetation indices here:
				# EVI
				x = G*(A[2]-A[1])/(A[2]+C1*A[1]-C2*A[4]+L)
				newM[iLon_[i],iLat_[i],d[3]] = oldM[iLon_[i],iLat_[i],d[3]] + (x-oldM[iLon_[i],iLat_[i],d[3]])/n
				newS[iLon_[i],iLat_[i],d[3]] = oldS[iLon_[i],iLat_[i],d[3]] + (x-oldM[iLon_[i],iLat_[i],d[3]])*(x-newM[iLon_[i],iLat_[i],d[3]])
				oldM[iLon_[i],iLat_[i],d[3]] = newM[iLon_[i],iLat_[i],d[3]]
				oldS[iLon_[i],iLat_[i],d[3]] = newS[iLon_[i],iLat_[i],d[3]]

				# NDVI
				NDVI[1] = (A[2]-A[1])/(A[2]+A[1])
				x = NDVI[1]
				newM[iLon_[i],iLat_[i],d[3]+1] = oldM[iLon_[i],iLat_[i],d[3]+1] + (x-oldM[iLon_[i],iLat_[i],d[3]+1])/n
				newS[iLon_[i],iLat_[i],d[3]+1] = oldS[iLon_[i],iLat_[i],d[3]+1] + (x-oldM[iLon_[i],iLat_[i],d[3]+1])*(x-newM[iLon_[i],iLat_[i],d[3]+1])
				oldM[iLon_[i],iLat_[i],d[3]+1] = newM[iLon_[i],iLat_[i],d[3]+1]
				oldS[iLon_[i],iLat_[i],d[3]+1] = newS[iLon_[i],iLat_[i],d[3]+1]
				#println(mat_data[iLon_[i],iLat_[i],d[3]+1] ," ", mat_data[iLon_[i],iLat_[i],2] )
				# NIRv
				x = NDVI[1]*A[2]*0.0001
				newM[iLon_[i],iLat_[i],d[3]+2] = oldM[iLon_[i],iLat_[i],d[3]+2] + (x-oldM[iLon_[i],iLat_[i],d[3]+2])/n
				newS[iLon_[i],iLat_[i],d[3]+2] = oldS[iLon_[i],iLat_[i],d[3]+2] + (x-oldM[iLon_[i],iLat_[i],d[3]+2])*(x-newM[iLon_[i],iLat_[i],d[3]+2])
				oldM[iLon_[i],iLat_[i],d[3]+2] = newM[iLon_[i],iLat_[i],d[3]+2]
				oldS[iLon_[i],iLat_[i],d[3]+2] = newS[iLon_[i],iLat_[i],d[3]+2]
				# NWDI
				x = (A[2]-A[5])/(A[2]+A[5])
				newM[iLon_[i],iLat_[i],d[3]+3] = oldM[iLon_[i],iLat_[i],d[3]+3] + (x-oldM[iLon_[i],iLat_[i],d[3]+3])/n
				newS[iLon_[i],iLat_[i],d[3]+3] = oldS[iLon_[i],iLat_[i],d[3]+3] + (x-oldM[iLon_[i],iLat_[i],d[3]+3])*(x-newM[iLon_[i],iLat_[i],d[3]+3])
				oldM[iLon_[i],iLat_[i],d[3]+3] = newM[iLon_[i],iLat_[i],d[3]+3]
				oldS[iLon_[i],iLat_[i],d[3]+3] = newS[iLon_[i],iLat_[i],d[3]+3]

			end
			oldM[iLon_[i],iLat_[i],end] +=   mat_in[idx[i][2],idx[i][1],end]
...
function main()
	...
	NCDictVar= Dict{String, NCDatasets.CFVariable}()
	...
	for (key, value) in dGrid
		...
		NCDictVar[key] = defVar(dsOut,string(key, "_std"),Float32,("time","lon","lat"),deflatelevel=4, fillvalue=-999)
	end
	...
	# Running mean and variance from Knuth (RunningStats)
    data_newM= zeros(Float32,(length(lon),length(lat),addData+1+length(dGrid)))
	data_oldM= zeros(Float32,(length(lon),length(lat),addData+1+length(dGrid)))
	data_newS= zeros(Float32,(length(lon),length(lat),addData+1+length(dGrid)))
	data_oldS= zeros(Float32,(length(lon),length(lat),addData+1+length(dGrid)))
	...
	for d in startDate:dDay:stopDate
	    ...
	    for a in files
	        ...
	        if length(idx) > 0
				...
				aver!(data_oldM,data_newM,data_oldS,data_newS, mat_in, iLon_,iLat_, idx)
				fill!(mat_in,0.0)
		...
		thr = 5
		for (key, value) in dGrid
			da = round.(data_newM[:,:,co],sigdigits=5)
			da_std = round.(sqrt.((data_newS[:,:,co]./NN)),sigdigits=5)
			#println(maximum(da), " ", maximum(NN))
			#da[NN.<1e-10].=-999
            da[NN.<thr].=-999

			da_std[NN.<thr].=-999
			da_std[data_newS[:,:,co].<0].=-999
			NCDict[key][cT,:,:]=da
			NCDictVar[key][cT,:,:]=da_std
			co += 1
		end
		d = size(mat_in)
		println(d)
		da = round.(data_oldM[:,:,d[3]],sigdigits=5)

		da[NN.<thr].=-999
		ds_evi[cT,:,:] = da
		da = round.(data_oldM[:,:,d[3]+1],sigdigits=5)
		da[NN.<thr].=-999
		ds_ndvi[cT,:,:] = da
		da = round.(data_oldM[:,:,d[3]+2],sigdigits=5)
		da[NN.<thr].=-999
		ds_nirv[cT,:,:] = da
		da = round.(data_oldM[:,:,d[3]+3],sigdigits=5)
		da[NN.<thr].=-999
		ds_ndwi[cT,:,:] = da
		cT += 1
		fill!(data_oldM,0.0)
