...
function aver!(mat_data, mat_in, iLon_,iLat_, idx)
	...
	for i = 1:length(idx)
		...
		if !any(A.==32767)
			...
			if(A[2]*0.0001>0.03)
				for j=1:d[3]-1
					@inbounds mat_data[iLon_[i],iLat_[i],j] +=  mat_in[idx[i][2],idx[i][1],j]*0.0001
				end
				# Compute vegetation indices here:
				# EVI
				@inbounds mat_data[iLon_[i],iLat_[i],d[3]] +=G*(A[2]-A[1])/(A[2]+C1*A[1]-C2*A[3]+L)
				# NDVI
				NDVI[1] = (A[2]-A[1])/(A[2]+A[1])
				#println(NDVI, " ", A[2]*0.0001, " ", A[1]*0.0001, " ", NDVI[1]*A[2]*0.0001," ", (A[2]*0.0001-A[1]*0.0001)/(A[2]*0.0001+A[1]*0.0001))

				@inbounds mat_data[iLon_[i],iLat_[i],d[3]+1] += NDVI[1]
				#println(mat_data[iLon_[i],iLat_[i],d[3]+1] ," ", mat_data[iLon_[i],iLat_[i],2] )
				# NIRv
				@inbounds mat_data[iLon_[i],iLat_[i],d[3]+2] += NDVI[1]*A[2]*0.0001
				# NWDI
				@inbounds mat_data[iLon_[i],iLat_[i],d[3]+3] +=(A[2]-A[5])/(A[2]+A[5])
				# N
