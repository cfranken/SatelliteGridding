...
function main()
    ...
    println("Output file dimension (lon/lat/time):")
    println(length(lon),"/", length(lat), "/", cT )
    # Create output file:
    ...
    for (key, value) in dGrid
        ...
        if ar["compSTD"]
            key2 = key*"_std"
            NCDict[key2] = defVar(dsOut,key2,Float32,("lon","lat","time"),deflatelevel=4, fillvalue=-999, comment="Standard Deviation from data")
        end
    end
    println(" ")
